...
- Read `${CODEX_HOME:-$HOME/.codex}/orchestra/roles.toml`. In dual mode, run one `session_model.py` check before tier selection and resolve `modes.<modelconfig>.tiers.<tier>.<capability>`; a legacy matrix resolves `tiers.<tier>.<capability>`. Root routing follows the detected model and multi-agent protocol, with Astra low as the native recommendation and the compatible Sol entry retained for legacy routing; effort alone never selects a mode. The selected model configuration is immutable for the task.
