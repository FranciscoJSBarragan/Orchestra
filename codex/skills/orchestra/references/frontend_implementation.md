...
- When visual iteration is needed, require `browser_route: auto | in_app | chrome`. Explicit user selection remains fixed unless fallback is authorized. `auto` explicitly selects Codex's in-app Browser first and may fall back to Computer Use with Chrome only for a technical availability or required-capability gap. `in_app` and `chrome` use only their selected surface.
