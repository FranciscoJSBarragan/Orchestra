...
- For `auto`, follow the host spawn reference: on Codex, explicitly select the dedicated Chrome connector first and fall back to Codex's in-app Browser only when Chrome is unavailable or has a technical capability gap that the in-app Browser can satisfy; Cursor and Grok Build map `auto` to Playwright.
- For `in_app`, use only Codex's in-app Browser; on Cursor or Grok return `blocked`. For `chrome`, use only the dedicated Chrome connector; on Grok return `blocked`. Do not substitute Computer Use or standalone browser automation. Cursor and Grok `auto` may use Playwright as the host-mapped surface.
