...
1. Resolve the requested executor, exact model, supported effort, checkout,
   ...
   --respect-workspace-trust false` in the checkout; omit `--model` only when
   the assignment means Devin's configured default. The helper pins
   `--permission-mode auto` (auto-approves read-only tools only) for read-only
   capabilities and under `default` permissions, and `dangerous`
   (auto-approves all tools) for implementation or verification capabilities
   under `--permissions trusted`. Each Devin delegation starts a fresh session
