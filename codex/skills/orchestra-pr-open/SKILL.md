...
4. Resolve the terminal phase commit from the completed manifest. Have the root directly run `python3 "${ORCHESTRA_HOME:-$HOME/.orchestra}/scripts/pr.py" open --repo <root> --repository <OWNER/REPO> --base <base> --head <head> --expected-task-revision <manifest-sha> --title <title> --body-file <body-file> --context-file <context-file> --authorized`. The helper must reload policy, allow only `pr-required` or `hybrid`, and require the supplied head to match the terminal manifest revision before any `gh` command.
