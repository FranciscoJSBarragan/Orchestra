...
4. Use `${CODEX_HOME:-$HOME/.codex}` as the installed Codex root, then have the root directly run `python3 "${CODEX_HOME:-$HOME/.codex}/orchestra/scripts/pr.py" open --repo <root> --repository <OWNER/REPO> --base <base> --head <head> --title <title> --body-file <body-file> --context-file <context-file> --authorized`. The helper must reload policy and allow only `pr-required` or `hybrid` before any `gh` command.
