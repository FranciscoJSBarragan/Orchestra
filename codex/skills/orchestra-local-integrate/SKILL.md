...
3. Use `${CODEX_HOME:-$HOME/.codex}` as the installed Codex root, then run `python3 "${CODEX_HOME:-$HOME/.codex}/orchestra/scripts/integrate_local.py" --task-worktree <task> --base-worktree <base> --task-branch <task-branch> --base-branch <base-branch> --authorized`.
