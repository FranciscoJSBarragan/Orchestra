...
3. Commit accepted fixes through [orchestra-phase-commit](../orchestra-phase-commit/SKILL.md), update the affected phase's terminal manifest commit, then have the root push the current branch directly under the existing `open PR` authority. The fix must remain inside approved intent; new scope requires a new task rather than a manifest update.
