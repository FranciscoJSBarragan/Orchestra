...
1. Have the root directly run `python3 "${ORCHESTRA_HOME:-$HOME/.orchestra}/scripts/pr.py" observe --repo <root> --repository <OWNER/REPO> --pr <number> [--previous-clean-head <sha>]`.
...
2. Have the same owner apply the fixes, rerun affected deterministic handoff checks, and publish a replacement `implementation-report`. Rerun any applicable independent gate with the same verifier, then send only the meaningful implementation delta and exact replacement evidence to the same reviewer. Require a current accepted `pr-review` before commit.
