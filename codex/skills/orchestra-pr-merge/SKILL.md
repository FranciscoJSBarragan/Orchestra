...
3. Use `${CODEX_HOME:-$HOME/.codex}` as the installed Codex root, then have the root directly run `python3 "${CODEX_HOME:-$HOME/.codex}/orchestra/scripts/pr.py" merge --repo <task-checkout> --base-worktree <base-checkout> --task-branch <task-branch> --base-branch <base-branch> --checkout-mode <managed|hybrid> [--start-revision <sha>] --remote <remote> --repository <OWNER/REPO> --pr <number> --clean-head <sha> --expected-task-revision <manifest-sha> --method <method> --authorized` and read its structured result.
4. Require the helper to load `<repo>/orchestra.toml` through `${CODEX_HOME:-$HOME/.codex}/orchestra/scripts/policy.py`, require a clean worktree, match the supplied terminal manifest revision, local HEAD, clean PR head, and current GitHub head, require a clean merge state, and run configured argv checks at the current repository.
