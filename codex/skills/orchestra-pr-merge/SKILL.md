...
1. Require separate explicit merge authority, checkout mode, the exact clean head, PR number, explicit `OWNER/REPO`, task/base checkout identity, task and base branch names, remote name, chosen merge method, and the captured starting revision for hybrid tasks.
2. Confirm [orchestra-pr-review](../orchestra-pr-review/SKILL.md) returned two complete clean observations on that same head.
3. Use `${CODEX_HOME:-$HOME/.codex}` as the installed Codex root, then have the root directly run `python3 "${CODEX_HOME:-$HOME/.codex}/orchestra/scripts/pr.py" merge --repo <task-checkout> --base-worktree <base-checkout> --task-branch <task-branch> --base-branch <base-branch> --checkout-mode <managed|hybrid> [--start-revision <sha>] --remote <remote> --repository <OWNER/REPO> --pr <number> --clean-head <sha> --method <method> --authorized` and read its structured result.
